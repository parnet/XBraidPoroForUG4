...
int main(int argc, char *argv[]) {
    ...
    if (run_wrapper_tests) {
        ...
        for (i = 0; i < 2; i++) {
            ...
            braid_TestResidual(app, comm_x, stdout, mytime, app->man->dt, my_Init, my_Access, my_Free, my_Clone, my_Sum,
                               my_SpatialNorm, my_Residual, my_Step);
            correct1 = braid_TestSpatialNorm(app, comm_x, stdout, mytime, my_Init, my_Free, my_Clone, my_Sum,
                                             my_SpatialNorm);
            correct2 = braid_TestBuf(app, comm_x, stdout, mytime, my_Init, my_Free, my_Sum, my_SpatialNorm, my_BufSize,
        ...
        if (myid == 0) {
            ...
            printf("  CFL ratio K (dt/dx^2 + dt/dy^2):  %1.2e\n\n",
                   app->man->K * ((app->man->dt) / ((app->man->dx) * (app->man->dx)) +
                                  (app->man->dt) / ((app->man->dy) * (app->man->dy))));
